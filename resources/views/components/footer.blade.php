    ...
    <div class="mx-auto flex max-w-6xl flex-col gap-6 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex flex-wrap gap-3">
            @foreach($socialLinks ?? [] as $item)
                <a href="{{ $item['url'] }}" target="_blank" rel="noopener noreferrer" class="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-semibold text-cream/80 transition hover:bg-sun hover:text-ink">
                    {{ $item['label'] }}
                </a>
            @endforeach
        </div>
